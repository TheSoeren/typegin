...
use core::{Action, Direction, GoTarget, Locator, parse_input};
...
mod parse {
    ...
    fn go_directions() {
        ...
        assert_eq!(
            Action::Go(GoTarget::Direction(Direction::South)),
            parses("go south")
        );
        assert_eq!(
            Action::Go(GoTarget::Direction(Direction::West)),
            parses("go west")
        );
    }
    ...
    fn take() {
        assert_eq!(
            Action::Take(Locator::Name("heavy iron key".to_string())),
    ...
    fn drop() {
        assert_eq!(
            Action::Drop(Locator::Name("iron key".to_string())),
    ...
    fn mixed_case_and_punctuation() {
        assert_eq!(
            Action::Examine(Locator::Name("glowing mysterious sword".to_string())),
