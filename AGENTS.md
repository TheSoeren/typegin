...
- `crates/core` — library (parsing, world state, persistence, rules, view trait)
...
| Regenerate diesel schema | `diesel print-schema > crates/core/src/schema.rs`                                                                       |
...
- `schema.rs` is **generated by Diesel CLI** — do not edit by hand. Regenerate with `diesel print-schema > crates/core/src/schema.rs` after migration changes.
- SQLite database created at `typegin.db` at runtime; override path with `TYPEGIN_DB` env var.
...
- `GameEngine::open` uses `BasicRules` (stock defaults). `open_with_rules` injects custom `Rules`. The terminal front-end in `src/main.rs` provides `TakeRules` that overrides `on_take` — the core's `BasicRules` does **not** move items into inventory.
